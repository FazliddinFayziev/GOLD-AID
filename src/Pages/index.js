

// USER RELATED PAGES
export { default as LessonsPage } from "./User/LessonsPage";
export { default as NewPassword } from "./User/NewPassword";
export { default as NotAllowed } from "./User/NotAllowed";
export { default as VideoPage } from "./User/VideoPage";
export { default as Register } from './User/Register';
export { default as Homework } from "./User/Homework";
export { default as TestPage } from "./User/TestPage";
export { default as Warning } from './User/Warning';
export { default as Profile } from "./User/Profile";
export { default as GoEmail } from "./User/GoEmail";
export { default as Offline } from "./User/Offline";
export { default as Already } from "./User/Already";
export { default as Forget } from "./User/Forget";
export { default as Passed } from "./User/Passed";
export { default as Failed } from "./User/Failed";
export { default as Login } from './User/Login';
export { default as Error } from "./User/Error";
export { default as Home } from './User/Home';
export { default as Layout } from "./Layout";

// ADMIN RELATED PAGES
export { default as Admin } from "./Admin/Admin";
export { default as Choose } from "./User/Choose";
export { default as NotAlailable } from "./Admin/NotAvailable";